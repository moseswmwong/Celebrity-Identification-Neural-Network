...
Motion provides a platform to support common webcam hardware on Linux platform with motion detection and image processing modules. I setup one web camera on the hardware with devices modules activated which supports PI cameras, V4L2 (Video for Linux) webcams,and movie file processing devices. 

Motion detection is achieved by comparing pixel differences and count-base threshold check, false positive was first filtering with noise level detection to avoid too much false motion identification generated by electric noise in the camera.
